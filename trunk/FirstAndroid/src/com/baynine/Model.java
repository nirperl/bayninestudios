...
public class Model
{
	...
	public Model(Context context, int xmlFile, int mType)
	{
		this(context, xmlFile, mType, 0f, 0f, 0f);
	}

	public Model(Context context, int xmlFile, int mType, float tx, float ty, float tz)
	{
		float[] coords = null;
		short[] icoords = null;
		float[] ncoords = null;
					...
					if (s.equals("position")) {
						...
						if (coords != null)
						{
							if (mType == 1)
							{
								...
								coords[vertexIndex++] = z + ty;
								coords[vertexIndex++] = y + tz;
							} else if (mType == 2) {
								coords[vertexIndex++] = x + tx;
	    ...
	    mNormalBuffer = FloatBuffer.wrap(ncoords);
